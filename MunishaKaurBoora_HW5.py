...
· Sprint retrospective
   - The sprint retrospective is a meeting held at the end of a sprint -  it is usually the last thing done in a sprint.
      - Many teams will hold a sprint retrospective immediately after the sprint review.
...
"""
User stories:
- As a yoga enthusiast, I want to be able to book yoga classes online in advance, so that I don't accidentally turn up to a full class again.
...
- As someone that is new to yoga, I want to be able to see all of the yoga studio's essential information clearly on the website, so that I can make an informed decision regarding whether or not I'd like to attend a yoga session.
...
-> The third user story requires a simple view of the data stored in the SQL database without overwhelming the individual.
...
- To build a simple prototype of a new yoga booking system, I would follow the agile methodology because it helps analyse and improve products whilst they’re going through different phases of development. 
...
"""
Key requirements:
- A database to store a user's name, surname, email address, the available films, the times that each film is showing, the various ticket prices, etc.
- A user interface that makes booking cinema tickets fast and simple (it should be faster and less cumbersome than booking tickets in person).
- The framework to build the user interface and connect to the database.
- The ability for the booking system to accept payment from the user and validate that payment is possible (the payment validation component could be external).

Main considerations:
- Will the booking system be used for one cinema or multiple cinemas operating under a particular brand or within a particular region?
...
- Booking tickets and users not receiving confirmation of the booking
- Users accidentally inputting the wrong data e.g the wrong email address.
- Users not wanting to use the website because they dont like the user interface or it is too difficult to navigate.
- The website taking a long time to load, leading to the user becoming frustrated.
- How many options should be displayed to the user when selecting films (a lot would be hard to display and may overwhelm the user).

Components or tools I would use:
- For the cinema booking system, I would use a strategy design pattern as it requires choosing a specific implementation of an algorithm or task at run time – out of multiple other implementations for the same task.
...
- I would use the Django python web framework.
   - I would use Django as it is well known to be a high-performing web framework out of the box and it is used by extremely high-traffic apps. This framework would help deal with the issue of alot of users attempting to book cinema tickets at the same time.
