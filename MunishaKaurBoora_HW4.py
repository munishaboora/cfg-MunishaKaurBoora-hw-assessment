...
- However, pin code is entered incorrectly once or twice to begin with and then entered correctly on the second or third attempt, the code in the else block will be executed and the second line of code in the finally block will also be executed. 
...
'''
 - I have refactored the code in task 2 and changed the names of the functions, from validate_available_balance and validate_pin, to validating_available_balance and validating_pin respectively.
    - I've used the original code within the the validate_available_balance function and included the account_balance variable within the function. I've also added an else statement with a return statement, and a return statement in place of the raise statement so that I can test the code.
